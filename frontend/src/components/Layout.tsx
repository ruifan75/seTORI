...
const adminItems = [
  ...
  { path: '/admin/aliases', label: '照合の学習', permission: PERM.CONTENT_EDIT },
                ...
                {status === 'authenticated' && user ? (
                  <div className="flex items-center justify-between gap-3">
                    <div className="flex items-center gap-3 min-w-0">
                      <Link to="/my/suggestions" className="text-sm text-gray-600 hover:text-indigo-600 truncate">
                        {user.display_name || user.username}
                        <span className="ml-1 text-xs text-gray-400">({user.role})</span>
                      </Link>
                      <Link to="/my/account" className="text-sm text-gray-500 hover:text-indigo-600 whitespace-nowrap">
                        アカウント
                      </Link>
                    </div>
                    <button
