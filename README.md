...
**公開サイト： [setori.app](https://setori.app)**
...
> 終了時間の由来（人手 / Holodex / コメント / 拍手検出 / iTunes 逆算 / 既定値）と、
> その確度をどう扱うか・誰がいつ確定させるかは
> **[`docs/DATA_COMPLETION.md`](./docs/DATA_COMPLETION.md)** にまとめています。

> AI 呼び出しは 1 配信あたり **1 回**。抽出・正規化・重複排除を統合したプロンプトで行い、
> 失敗時は 2 段階（抽出→正規化）→ 純正規表現へ段階的に退避します。
> 実際に送っている prompt と返ってくる JSON の形式、実採取した入出力の例、
> トークン量と費用の実測、モデル差への対応、既知の問題は
...
Copyright 2026 ruifan75.

特に記載がない限り、このリポジトリのソースコードとプロジェクト文書は
[Apache License 2.0](./LICENSE) の下で提供されます。

外部サービスから取得・派生したデータ、画像、動画、音源、名称、ロゴなどの第三者コンテンツは
このライセンスの対象外であり、それぞれの権利者および利用条件に従います。
