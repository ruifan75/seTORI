...
- a: **原曲のアーティスト名**。ここは逐字ではありません。
  **コメントに書かれていなくても、その曲の原曲アーティストが分かるなら補ってください。**
  例: "20:42 群青" だけの行でも a は "YOASOBI"、"32:30 秒針を噛む" なら "ずっと真夜中でいいのに。"。
  これは既存の楽曲データベースとの照合に使う項目で、**空にすると照合できません**。
