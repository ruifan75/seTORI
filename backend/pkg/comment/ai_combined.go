...
各要素の形式:
{"line":行番号,"is_song":true/false,"start_ts":"...","end_ts":"...","name_verbatim":"...","artist_verbatim":"...","normalized_name":"...","normalized_name_reading":"...","normalized_artist":"...","normalized_artist_reading":"...","tags":[],"confidence":0.0}

## 判定

- is_song: 実際に歌唱された曲なら true。以下はすべて false:
  - 雑談・開演・終了・告知・スパチャ読み
  - 「トピック 「発言内容」」のような実況メモ（行内に曲名が引用されていても話題であって歌唱ではない）
  - 絵文字や記号だけの行、リスナーの感想（「ここ好き」「高音が綺麗」など）
  - 罫線（┗ ┣ ┏ など）でぶら下げた注釈行
- is_song=false のときは name/artist/normalized 系はすべて空文字列、tags は空配列にする。

## 逐字フィールド（name_verbatim / artist_verbatim）
...
start_ts / end_ts には**入力行に実際に書かれている時刻文字列をそのままコピー**します
（例 "01:12:42" "58:35"）。秒数に変換してはいけません。1行に2つあれば順に start_ts / end_ts へ。
is_song=false の行でも、時刻があれば start_ts は入れてください。

## 正規化フィールド

- normalized_name: 演奏バージョンの表記（Acoustic Ver., Short Ver., Piano ver., アカペラ など
  **演奏方法**を示すもの）を取り除いた曲名。
...
- normalized_artist: アーティスト名。**演奏方法を表す語をアーティストにしてはいけません。**
  例えば "『愛・おぼえていますか』（アカペラ）" の「アカペラ」はアーティストではなくタグです。
- *_reading: 平仮名の読み。不明なら空文字列。
- confidence: 0.0〜1.0。
...
[
  {"line":1,"is_song":true,"start_ts":"1:46:35","end_ts":"","name_verbatim":"愛・おぼえていますか","artist_verbatim":"","normalized_name":"愛・おぼえていますか","normalized_name_reading":"あいおぼえていますか","normalized_artist":"","normalized_artist_reading":"","tags":["acappella"],"confidence":0.9},
  {"line":2,"is_song":false,"start_ts":"0:00:00","end_ts":"","name_verbatim":"","artist_verbatim":"","normalized_name":"","normalized_name_reading":"","normalized_artist":"","normalized_artist_reading":"","tags":[],"confidence":0.0}
]`
...
type combinedSelection struct {
	...
	NameVerb  string   `json:"name_verbatim"`
	ArtistVer string   `json:"artist_verbatim"`
	NormName  string   `json:"normalized_name"`
	NormRead  string   `json:"normalized_name_reading"`
	NormArt   string   `json:"normalized_artist"`
	NormArtRd string   `json:"normalized_artist_reading"`
...
func filterAllowedTags(tags []string) []string {
	...
	for _, t := range tags {
		...
		if !allowedTags[t] {
			logger.Debugf("AI combined: dropping unknown tag %q", t)
			continue
		}
		seen[t] = true
		out = append(out, t)
