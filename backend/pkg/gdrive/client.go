...
//
// ⚠️ **「2,000 件までは取れる」という保証ではない。** Drive は pageSize より
// 少ない件数でページを区切ることがあるので、上限に当たる実際の件数はもっと
// 少なくなりうる。そして**当たると管理画面の一覧も失敗する**（同じ関数を通る）ので、
// 復旧には Drive 側で手で片付ける必要がある。印なし・使わなくなった印の
// ファイルは世代整理の対象外なので、そこが溜まる経路になる。
