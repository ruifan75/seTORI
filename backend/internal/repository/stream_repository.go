...
func (r *StreamRepository) GetSingersForStreams(streamIDs []string) (participants map[string][]models.Singer, owners map[string]*models.Singer, err error) {
	...
	query := `
		SELECT ss.stream_id, ss.is_owner, s.id, s.name, s.english_name, s.photo_url, s.organization, o.display_name, s.metadata_source, s.created_at, s.updated_at
		FROM singers s
	...
	for rows.Next() {
		...
		if err := rows.Scan(&streamID, &isOwner, &sg.ID, &sg.Name, &sg.EnglishName,
			&sg.PhotoURL, &sg.Organization, &sg.OrganizationName, &sg.MetadataSource, &sg.CreatedAt, &sg.UpdatedAt); err != nil {
...
func (r *StreamRepository) GetChannelOwner(streamID string) (*models.Singer, error) {
	...
	var s models.Singer
	err := r.db.QueryRow(query, streamID).Scan(&s.ID, &s.Name, &s.EnglishName, &s.PhotoURL, &s.Organization, &s.OrganizationName, &s.MetadataSource, &s.CreatedAt, &s.UpdatedAt)
	if err == sql.ErrNoRows {
