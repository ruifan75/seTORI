...
--
--   入力元にあり DB に無い       → 審査へ（perf.missing 提案）
--   両方にあり内容が違う     → 審査へ（conflict）
--   両方にあり一致           → 何もしない
--   **DB にあり入力元に無い**    → 数えてログに出すだけで、画面から見えなかった
--
-- 最後のものを提案として積まないのは今も同じ。入力元は欠けているのが普通で、欠落 1 件ごとに
-- 待ち行列を作ると人が処理できない量になり、しかも「入力元に無い」だけでは何をすべきか
...
COMMENT ON TABLE batch_fill_gaps IS 'force 実行で「DB にあるが入力元に無い」と分かった歌唱';
COMMENT ON COLUMN batch_fill_runs.songs_gap IS '入力元に無かった既存の歌唱の件数';
