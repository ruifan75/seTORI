...
> - ⚠️ **Drive の連携設定も一緒に来る。** `drive_upload` と `drive_folder_id` は
>   `app_settings` にあるので、復元した開発環境が**本番と同じフォルダへ上げ始める**。
>   開発環境でバックアップを Drive へ置く必要が無いなら、復元後に
>   `管理 → バックアップ` で Drive アップロードを切ること。
>   （世代整理そのものは環境ごとの印で分離されるので、本番のバックアップを
>   消すことは無い。ただしフォルダは共有され、保持数の枠を食い合う）
...
#### Drive 上のファイルには「どの環境が作ったか」の印が付く

Drive のフォルダは**名前で解決される**ので、同じ Google アカウントを使う限り
本番も開発環境も**同じフォルダ**に行き着く。世代整理（保持数を超えたぶんの削除）が
出所を見ないと、**開発環境のバックアップが本番のバックアップを消す**。

そこで Drive 上の名前を `<印>__setori_YYYYMMDD_HHMMSS.dump` にし、整理は
**その印が一致するものだけ**を対象にする。印は `ENVIRONMENT` から取る。

| 状況 | 世代整理の扱い |
|---|---|
| 印が一致 | 保持数を超えたぶんを削除 |
| 別の環境の印 | 触らない |
| 印が無い（この仕組みより前に上がったもの） | 触らない |
| `ENVIRONMENT` が未設定 | **1 件も消さない** |

> ⚠️ **`ENVIRONMENT` を設定しないと世代整理が止まる**（消さない側へ倒れるので
> 安全だが、溜まり続ける）。管理画面のバックアップ節にこの環境の印が出るので、
> 意図した値になっているか確かめること。
>
> **印は `app_settings` に保存しない。** そこへ置くと pg_dump で複製され、
> 本番を復元した開発環境が「production」を名乗る ── 防ごうとしている事故を
> そのまま起こす。だから環境変数からしか読まない。
>
> 印が無い状態で溜まったファイルは自動では消えない。手で片付けること。
