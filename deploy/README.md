...
> - **開発環境と同じ鍵を使っている（意図的）。** 分けると、本番のダンプを手元へ
>   復元するたびに API キーを入れ直すことになる。忘れると Holodex 同期や AI 解析が
>   静かに失敗するので、**忘れうる手動手順そのものが故障要因**になる。
>   両環境が同じ API キーを持っている以上、鍵を分けても攻撃者が得られるものは
>   増えないため、この取引は割に合うと判断した
>   - 代償：鍵の更新が両環境で同期必須になり、更新前のバックアップは
>     新しい鍵では復号できなくなる
>   - **本番だけが持つ秘密ができたら考え直すこと**（別の Holodex アカウント、
>     課金系の資格情報など）。そのときは隔離に意味が出てくる
