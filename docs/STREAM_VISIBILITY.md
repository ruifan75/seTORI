...
> ⚠️ **SQL の `NotRestricted` と Go の `effectiveRestricted` は同じ規則。片方だけ変えないこと。**
> 食い違うと「一覧には出ないのに詳細では出る」になる。
> 実測（2026-08-29）：チャンネルに `allow` を設定すると、詳細でも曲ページでも
> 歌唱が出るようになり、`deny` に変えると両方から消える。
