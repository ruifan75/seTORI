...
（元から向こうにあった曲も多い）、機械的には区別できない。

**2 つ目の材料でも旧行は拾えない。** `SyncSetoriToHolodex` は PUT の結果を
`holodex_data` へ書き戻さないので、送信に成功しても保存済みのスナップショットは空のまま。
つまり「台帳も曲も無い」旧行が実在し、そこを無警告にすると
**警告が無いことを安全の根拠にできない**。

そこで 3 つ目に `streams.holodex_upload_unknown`（migration 055）を置く。
「この行は追跡の開始より前から存在する」という事実そのもので、
新しい行は既定の `FALSE` で入る（＝台帳が空なら本当に送っていない）。
**`TRUE` は以後増えないが、自動では減りもしない** ── 解除する UPDATE は無いので、
既存行はその配信が消えるまで保守的な印を持ち続ける。
「確認したので外す」という運用が要るなら別 issue。
実測：適用時点の 1307 行が `TRUE`、そのあと作った行は `FALSE`。
...
> **2026-09-14 に取得をやめた（PR #66）。** 3 列は残してあるが、書き込む経路は無く、
> 行を読むときに SELECT・Scan はされるものの**判定には一切使わない**。会限の検出に 0 件しか寄与しておらず（本番 86 件はすべて
...
⚠️ **画面が先に案内へ倒すのは `members_only` だけ**（2026-09-05）。他の値は
編集者向けの情報として応答に載るが、プレイヤーの描画は止めない。理由は
「再生可否は見る人の所在地で変わる」── 詳細は後述の「① 描画時に実測へ倒す」。
...
**②へ進むのは①が終了コード 0 だったときだけ。** ①が非ゼロなら、その失敗は
フラグの有無では解けない（BOT 判定・レート制限・タイムアウト・未インストール）。
もう一度走らせても同じ理由で落ち、障害時に外部呼び出しが倍になる
（1306 件なら最大 2612 回、タイムアウトなら 1 件 4 分）。

しかも危険がある：①が「レート制限」と**確定できていた**のに、その証拠を捨てて
②の短い応答だけを見ると、同じ動画を `failureVideoGone` として `unavailable` に
確定させうる。`classifyFetchFailure` が「transient を先に見る」ようにしてある意味が、
run をまたぐと失われる。

**②へ来たら①の結果は丸ごと捨てること。** ①の `public` は反証を取りに行けていないだけの
嘘で、持ち越すと「動画が無い」の保存にその嘘が混ざる（最初の実装がそうなっていて、
`hVfDBfreYNI` が `availability=public` のまま保存された）。
**値だけでなく `cookiesSent` も**。1 つだけ①のものを見ていると、
「①は cookie あり・②は cookie 無し」のときに会限を「動画消失」と確定させる
── そのため実行結果は struct（`ytdlpRun`）で持ち、まとめて差し替える。

②が走るのは①が終了コード 0 で値が揃わなかったときだけなので、
追加のコストは削除済みの少数に限られる。
...
専用の `Fetch` は①でフラグを付け、値が揃わなければ②でフラグを外す（前述の 2 段構え）。
②はフラグが無いので、取れなければ非ゼロで終わる。判定はそのうえで：

1. ①か②が終了コード 0 かつ `Resolved()`（両方の値が揃った）→ そのまま保存
2. ②が非ゼロ ＋ **②で** cookie を実際に渡した ＋ **一時障害ではない** ＋
   stderr が「動画が無い」と読める → `unavailable` として保存
3. それ以外 → **保存しない**（未調査のまま残り、次の backfill が拾い直す）

2 の「②で cookie を渡した」が要点。①の `cookiesSent` を見てはいけない
（cookie の設定が消える／②の一時ファイル作成だけ失敗する、で食い違いうる）。

#### `public` は「反証が無かった」であって、公開だと確かめた証拠ではない

これは**どの経路でも塞げない**。`_availability`（`common.py`）は 5 つの材料が
...
なお `Fetch` は①でフラグを付け、②で外す（上記の 2 段構え）。
...
**そして、先に案内へ倒すのは会限だけにする**（2026-09-05）。以前は `playable` /
`unknown` 以外なら描画そのものを止めていたが、**yt-dlp は東京の VPS で走る**ので
その結論は「東京から見て再生できるか」でしかない。日本で権利上ブロックされていても
他の地域では再生できる動画があり、先に倒すとその人から再生を奪う ── しかも
**プレイヤーを描かないので `onError` は永遠に鳴らず、取り返す機会が無い**。

誤りが出るのは「先に判定する」側だけなのが要点で、逆向き（東京では再生できるが
他の地域では不可）は `onError` が拾う。**非対称なので、迷ったら描いてみるほうへ倒す。**

例外が会限なのは、所在地に依らず 150 を返すと実測できているため（メンバー資格の
あるアカウントでも同じ）。必ず失敗する iframe を描くより理由を出すほうがよい。
...
>
> **cancel の保証は「応答のあと新しい 1 件は始まらない」。**
> 既に始まっているものは最後まで走る（最大で並列数ぶん）。途中で殺さないのは
> yt-dlp の一時ファイルが残るため。
>
> 保証の作り方：**cancel の確認と起動の予約を、Cancel と同じ mutex の下で行う。**
> 確認だけを別に取ると、解錠してから `go` するまでの間に cancel が入り、
> その 1 件が cancel の応答後に開始してしまう（semaphore を待っていた 1 件が典型）。
