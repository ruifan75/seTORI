...
| `GET /api/streams/{id}` | 非表示配信でも 200。`performances` も含む |
| `PerformanceRepository.FindByStreamID` | `WHERE p.stream_id = $1` のみ。非表示条件なし |
| `PerformanceRepository.FindByID` | `WHERE p.id = $1` のみ。`GET /api/performances/{id}` は公開 |
| `PlaylistRepository.ListItems` | **意図的に残す**（「勝手に消えると混乱するため」） |
